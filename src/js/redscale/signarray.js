...
redscale.SignArray.signAdd = function( aArray, bArray ) {
  ...
  if ( aArray.sign === 0 ) {
    ...
    aArray.array = redscale.util.copy( bArray.array, 0, new Int16Array( bArray.array.length ), 0, bArray.array.length );

    return bArray;
  }
  if ( bArray.sign === 0 ) { return aArray; }
  if ( aArray.sign === bArray.sign ) {
    aArray.array = redscale.arithmetic.add( aArray.array, bArray.array );
  } else {
    abComp = redscale.util.compare( aArray.array, bArray.array );
    ...
    if ( abComp === 0 ) {
      ...
      aArray.array = redscale.arithmetic.subtract( aArray.array, bArray.array );
    } else {
      aArray.sign = abComp === aArray.sign ? 1 : -1;
      aArray.array = redscale.arithmetic.subtract( bArray.array, aArray.array );
    }
  }
